...
#
# Tests report generation for s3 data. Returns 0 for success, 1 for failure.
#
# Author: Tom Werges
#
...
wget -O /tmp/s3Report.csv --no-check-certificate "https://$CLC_IP:8443/reports?session=$SESSIONID&name=user_s3&type=csv&page=0&flush=false&start=1104580000000&end=1104590000000&criterionId=2&groupById=0"
...
REPORT_USERS_CNT=`cat /tmp/s3Report.csv |  grep 'user-[0-9]\+' | wc -l`
if [ "$REPORT_USERS_CNT" -ne "32" ]; then
	echo "Report users count is incorrect."
	exit -1
fi

# Verify that the data in the report is correct, using the CsvChecker tool.
java -jar CsvChecker.jar 0.2 s3Reference.csv /tmp/s3Report.csv
