...
static int arguments_internal(int argc, char **argv, struct eucalyptus_opts *args_info, struct arguments_params *params, const char *additional_error);

static int arguments_required2(struct eucalyptus_opts *args_info, const char *prog_name, const char *additional_error);
...
{
	printf("%s %s\n", (strlen(ARGUMENTS_PACKAGE_NAME) ? ARGUMENTS_PACKAGE_NAME : ARGUMENTS_PACKAGE), ARGUMENTS_VERSION);
}
...
{
	struct arguments_params *params = (struct arguments_params *)malloc(sizeof(struct arguments_params));
...
static void arguments_release(struct eucalyptus_opts *args_info)
{
...
int check_multiple_option_occurrences(const char *prog_name, unsigned int option_given, unsigned int min, unsigned int max, const char *option_desc)
{
	...
	if (option_given && (min > 0 || max > 0)) {
		if (min > 0 && max > 0) {
			...
			} else if (option_given < (unsigned int)min || option_given > (unsigned int)max) {
				/* range occurrences */
...
int arguments_required2(struct eucalyptus_opts *args_info, const char *prog_name, const char *additional_error)
{
	int error = 0;
...
int update_arg(void *field, char **orig_field,
			   ...
			   arguments_arg_type arg_type, int check_ambiguity, int override, int no_free, int multiple_option, const char *long_opt, char short_opt, const char *additional_error)
{
	char *stop_char = 0;
...
int update_multiple_arg_temp(struct generic_list **list,
							 ...
							 const char *possible_values[], const char *default_value, arguments_arg_type arg_type, const char *long_opt, char short_opt, const char *additional_error)
{
	...
	while (1) {
		add_node(list);
		if (update_arg((void *)&((*list)->arg), &((*list)->orig), 0, prev_given, multi_token, possible_values, default_value, arg_type, 0, 1, 1, 1, long_opt, short_opt, additional_error)) {
			if (multi_token)
				free(multi_token);
			return 1;			/* failure */
		}
 ...
 */
static
void update_multiple_arg(void *field, char ***orig_field, unsigned int field_given, unsigned int prev_given, union generic_value *default_value, arguments_arg_type arg_type, struct generic_list *list)
{
	int i;
	struct generic_list *tmp;
...
int arguments_internal(int argc, char **argv, struct eucalyptus_opts *args_info, struct arguments_params *params, const char *additional_error)
{
	...
	while (1) {
		...
		case 'u':				/* User to drop privs to after starting..  */
			...
			if (update_arg((void *)&(args_info->user_arg),
						   ...
						   &(local_args_info.user_given), optarg, 0, "eucalyptus", ARG_STRING, check_ambiguity, override, 0, 0, "user", 'u', additional_error))
				goto failure;
		...
		case 'h':				/* Eucalyptus home directory..  */
			...
			if (update_arg((void *)&(args_info->home_arg),
						   ...
						   &(local_args_info.home_given), optarg, 0, "/opt/grze_eee/", ARG_STRING, check_ambiguity, override, 0, 0, "home", 'h', additional_error))
				goto failure;
		...
		case 'i':				/* Specifying this option causes eucalyptus-cloud to only bind the specified local addresses.  The default behaviour is to listen on the any address while determining the user facing local address based on default route and netmask size..  */
			...
			if (update_multiple_arg_temp(&bind_addr_list, &(local_args_info.bind_addr_given), optarg, 0, 0, ARG_STRING, "bind-addr", 'i', additional_error))
				goto failure;
		...
		case 'b':				/* Host to be used for bootstrapping group membership.  Many can be provided.  Note this should only be necessary when UDP multicast is not available..  */
			...
			if (update_multiple_arg_temp(&bootstrap_host_list, &(local_args_info.bootstrap_host_given), optarg, 0, 0, ARG_STRING, "bootstrap-host", 'b', additional_error))
				goto failure;

			break;
		case 'D':				/* Set system properties..  */

			if (update_multiple_arg_temp(&define_list, &(local_args_info.define_given), optarg, 0, 0, ARG_STRING, "define", 'D', additional_error))
				goto failure;
		...
		case 'f':				/* Fork and daemonize Eucalyptus..  */
			...
			if (update_arg((void *)&(args_info->fork_flag), 0, &(args_info->fork_given),
						   &(local_args_info.fork_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "fork", 'f', additional_error))
				goto failure;
		...
		case 'k':				/* Kill a daemonized Eucalyptus..  */
			...
			if (update_arg((void *)&(args_info->kill_flag), 0, &(args_info->kill_given),
						   &(local_args_info.kill_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "kill", 'k', additional_error))
				goto failure;
		...
		case 'l':				/* Control the log level for console output..  */
			...
			if (update_arg((void *)&(args_info->log_level_arg),
						   ...
						   &(local_args_info.log_level_given), optarg, 0, "INFO", ARG_STRING, check_ambiguity, override, 0, 0, "log-level", 'l', additional_error))
				goto failure;
		...
		case 'L':				/* Control the destination for console output..  */
			...
			if (update_arg((void *)&(args_info->log_appender_arg),
						   ...
						   &(local_args_info.log_appender_given), optarg, 0, "console-log", ARG_STRING, check_ambiguity, override, 0, 0, "log-appender", 'L', additional_error))
				goto failure;
		...
		case 'x':				/* Exhaustive logging of all connections..  */
			...
			if (update_arg((void *)&(args_info->exhaustive_flag), 0, &(args_info->exhaustive_given),
						   &(local_args_info.exhaustive_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "exhaustive", 'x', additional_error))
				goto failure;
		...
		case 'o':				/* Redirect standard out to file..  */
			...
			if (update_arg((void *)&(args_info->out_arg),
						   &(args_info->out_orig), &(args_info->out_given), &(local_args_info.out_given), optarg, 0, "&1", ARG_STRING, check_ambiguity, override, 0, 0, "out", 'o', additional_error))
				goto failure;
		...
		case 'e':				/* Redirect standard error to file..  */
			...
			if (update_arg((void *)&(args_info->err_arg),
						   &(args_info->err_orig), &(args_info->err_given), &(local_args_info.err_given), optarg, 0, "&2", ARG_STRING, check_ambiguity, override, 0, 0, "err", 'e', additional_error))
				goto failure;

			break;
		case 'j':				/* Alternative way to specify JAVA_HOME..  */

			if (update_multiple_arg_temp(&java_home_list, &(local_args_info.java_home_given), optarg, 0, "/usr/lib/jvm/java-6-openjdk/", ARG_STRING, "java-home", 'j', additional_error))
				goto failure;

			break;
		case 'X':				/* Arguments to pass to the JVM..  */

			if (update_multiple_arg_temp(&jvm_args_list, &(local_args_info.jvm_args_given), optarg, 0, 0, ARG_STRING, "jvm-args", 'X', additional_error))
				goto failure;
		...
		case 'v':				/* Launch the JVM w/ verbose output flags..  */
			...
			if (update_arg((void *)&(args_info->verbose_flag), 0, &(args_info->verbose_given),
						   &(local_args_info.verbose_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "verbose", 'v', additional_error))
				goto failure;
		...
		case 'H':				/* Launch with CLC in debug mode (no state management)..  */
			...
			if (update_arg((void *)&(args_info->debug_noha_flag), 0, &(args_info->debug_noha_given),
						   &(local_args_info.debug_noha_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "debug-noha", 'H', additional_error))
				goto failure;
		...
		case 0:				/* Long option with no short option */
			...
			if (strcmp(long_options[option_index].name, "extra-version") == 0) {
				...
				if (update_arg((void *)&(args_info->extra_version_arg),
							   ...
							   &(local_args_info.extra_version_given), optarg, 0, "", ARG_STRING, check_ambiguity, override, 0, 0, "extra-version", '-', additional_error))
					goto failure;
			...
			/* Perform first-time setup.  This is run one time (only!) on the first cloud controller before the first time it is run..  */
				...
				if (update_arg((void *)&(args_info->initialize_flag), 0, &(args_info->initialize_given),
							   &(local_args_info.initialize_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "initialize", '-', additional_error))
					goto failure;
			...
			/* Perform database upgrade procedure and exit immediately after completion.  Normally, upgrade is performed in-line with regular execution.  This flag causes upgrade to be run and then for the process to terminate.  Useful for debugging upgrade failures..  */
				...
				if (update_arg((void *)&(args_info->upgrade_flag), 0, &(args_info->upgrade_given),
							   &(local_args_info.upgrade_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "upgrade", '-', additional_error))
					goto failure;
			...
			/* Upgrade from specified version..  */
				...
				if (update_arg((void *)&(args_info->upgrade_old_version_arg),
							   ...
							   &(local_args_info.upgrade_old_version_given), optarg, 0, 0, ARG_STRING, check_ambiguity, override, 0, 0, "upgrade-old-version", '-', additional_error))
					goto failure;
			...
			/* Upgrade from specified directory..  */
				...
				if (update_arg((void *)&(args_info->upgrade_old_dir_arg),
							   ...
							   &(local_args_info.upgrade_old_dir_given), optarg, 0, 0, ARG_STRING, check_ambiguity, override, 0, 0, "upgrade-old-dir", '-', additional_error))
					goto failure;
			...
			/* Skip version check to force upgrade to run again..  */
				...
				if (update_arg((void *)&(args_info->upgrade_force_flag), 0, &(args_info->upgrade_force_given),
							   &(local_args_info.upgrade_force_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "upgrade-force", '-', additional_error))
					goto failure;
			...
			/* Force the system to boot as a remote component..  */
				...
				if (update_arg((void *)&(args_info->force_remote_bootstrap_flag), 0, &(args_info->force_remote_bootstrap_given),
							   &(local_args_info.force_remote_bootstrap_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "force-remote-bootstrap", '-', additional_error))
					goto failure;
			...
			/* Location for the pidfile..  */
				...
				if (update_arg((void *)&(args_info->pidfile_arg),
							   ...
							   check_ambiguity, override, 0, 0, "pidfile", '-', additional_error))
					goto failure;
			...
			/* Set path to database home directory.  */
				...
				if (update_arg((void *)&(args_info->db_home_arg),
							   ...
							   &(local_args_info.db_home_given), optarg, 0, "", ARG_STRING, check_ambiguity, override, 0, 0, "db-home", '-', additional_error))
					goto failure;
			...
			/* Exhaustive logging for database connections..  */
				...
				if (update_arg((void *)&(args_info->exhaustive_db_flag), 0, &(args_info->exhaustive_db_given),
							   &(local_args_info.exhaustive_db_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "exhaustive-db", '-', additional_error))
					goto failure;
			...
			/* Exhaustive logging for client connections..  */
				...
				if (update_arg((void *)&(args_info->exhaustive_user_flag), 0, &(args_info->exhaustive_user_given),
							   &(local_args_info.exhaustive_user_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "exhaustive-user", '-', additional_error))
					goto failure;
			...
			/* Exhaustive logging for cluster connections..  */
				...
				if (update_arg((void *)&(args_info->exhaustive_cc_flag), 0, &(args_info->exhaustive_cc_given),
							   &(local_args_info.exhaustive_cc_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "exhaustive-cc", '-', additional_error))
					goto failure;
			...
			/* Exhaustive logging for external libraries..  */
				...
				if (update_arg((void *)&(args_info->exhaustive_external_flag), 0, &(args_info->exhaustive_external_given),
							   &(local_args_info.exhaustive_external_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "exhaustive-external", '-', additional_error))
					goto failure;
			...
			/* eucalyptus-cloud will not try to bind port 53..  */
				...
				if (update_arg((void *)&(args_info->remote_dns_flag), 0, &(args_info->remote_dns_given),
							   &(local_args_info.remote_dns_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "remote-dns", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->remote_cloud_flag), 0, &(args_info->remote_cloud_given),
							   &(local_args_info.remote_cloud_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "remote-cloud", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->remote_walrus_flag), 0, &(args_info->remote_walrus_given),
							   &(local_args_info.remote_walrus_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "remote-walrus", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->remote_storage_flag), 0, &(args_info->remote_storage_given),
							   &(local_args_info.remote_storage_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "remote-storage", '-', additional_error))
					goto failure;
			...
			/* Disable ISCSI support for dynamic block storage..  */
				...
				if (update_arg((void *)&(args_info->disable_iscsi_flag), 0, &(args_info->disable_iscsi_given),
							   &(local_args_info.disable_iscsi_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "disable-iscsi", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->disable_cloud_flag), 0, &(args_info->disable_cloud_given),
							   &(local_args_info.disable_cloud_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "disable-cloud", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->disable_walrus_flag), 0, &(args_info->disable_walrus_given),
							   &(local_args_info.disable_walrus_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "disable-walrus", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->disable_dns_flag), 0, &(args_info->disable_dns_given),
							   &(local_args_info.disable_dns_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "disable-dns", '-', additional_error))
					goto failure;
			...
			/* DEPRECATED DO NOT USE. IT DOES NOTHING..  */
				...
				if (update_arg((void *)&(args_info->disable_storage_flag), 0, &(args_info->disable_storage_given),
							   &(local_args_info.disable_storage_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "disable-storage", '-', additional_error))
					goto failure;
			...
			/* Which JVM type to run (see jvm.cfg)..  */
				...
				if (update_arg((void *)&(args_info->jvm_name_arg),
							   ...
							   &(local_args_info.jvm_name_given), optarg, 0, "-server", ARG_STRING, check_ambiguity, override, 0, 0, "jvm-name", '-', additional_error))
					goto failure;
			...
			/* Launch with JMX enabled..  */
				...
				if (update_arg((void *)&(args_info->jmx_flag), 0, &(args_info->jmx_given),
							   &(local_args_info.jmx_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "jmx", '-', additional_error))
					goto failure;
			...
			/* Set the port to use for the debugger..  */
				...
				if (update_arg((void *)&(args_info->debug_port_arg),
							   ...
							   &(local_args_info.debug_port_given), optarg, 0, "5005", ARG_INT, check_ambiguity, override, 0, 0, "debug-port", '-', additional_error))
					goto failure;
			...
			/* Force debugger to wait at main()..  */
				...
				if (update_arg((void *)&(args_info->debug_suspend_flag), 0, &(args_info->debug_suspend_given),
							   &(local_args_info.debug_suspend_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "debug-suspend", '-', additional_error))
					goto failure;
			...
			/* Launch with jprofiler enabled..  */
				...
				if (update_arg((void *)&(args_info->profile_flag), 0, &(args_info->profile_given),
							   &(local_args_info.profile_given), optarg, 0, 0, ARG_FLAG, check_ambiguity, override, 1, 0, "profile", '-', additional_error))
					goto failure;
			...
			/* Set the home for jprofiler..  */
				...
				if (update_arg((void *)&(args_info->profiler_home_arg),
							   ...
							   &(local_args_info.profiler_home_given), optarg, 0, "/opt/profile", ARG_STRING, check_ambiguity, override, 0, 0, "profiler-home", '-', additional_error))
					goto failure;
			...
			/* Launch with agentlib enabled..  */
				...
				if (update_arg((void *)&(args_info->agentlib_arg),
							   ...
							   &(local_args_info.agentlib_given), optarg, 0, 0, ARG_STRING, check_ambiguity, override, 0, 0, "agentlib", '-', additional_error))
					goto failure;
	...
	update_multiple_arg((void *)&(args_info->bind_addr_arg), &(args_info->bind_addr_orig), args_info->bind_addr_given, local_args_info.bind_addr_given, 0, ARG_STRING, bind_addr_list);
	update_multiple_arg((void *)&(args_info->bootstrap_host_arg),
						&(args_info->bootstrap_host_orig), args_info->bootstrap_host_given, local_args_info.bootstrap_host_given, 0, ARG_STRING, bootstrap_host_list);
	update_multiple_arg((void *)&(args_info->define_arg), &(args_info->define_orig), args_info->define_given, local_args_info.define_given, 0, ARG_STRING, define_list);
	multiple_default_value.default_string_arg = "/usr/lib/jvm/java-6-openjdk/";
	update_multiple_arg((void *)&(args_info->java_home_arg),
						&(args_info->java_home_orig), args_info->java_home_given, local_args_info.java_home_given, &multiple_default_value, ARG_STRING, java_home_list);
	update_multiple_arg((void *)&(args_info->jvm_args_arg), &(args_info->jvm_args_orig), args_info->jvm_args_given, local_args_info.jvm_args_given, 0, ARG_STRING, jvm_args_list);
