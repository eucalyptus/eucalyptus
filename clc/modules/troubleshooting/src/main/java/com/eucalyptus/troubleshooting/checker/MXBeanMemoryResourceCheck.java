					...
					try {
						...
						if (free < mxBeanInfo.getThreshold(bean)) {
							if (!this.alreadyFaulted.contains(identifier)) {
								Faults.forComponent(Eucalyptus.class).havingId(MXBEAN_OUT_OF_MEMORY_FAULT_ID).withVar("component", Eucalyptus.INSTANCE.getFaultLogPrefix()).withVar("name",  name).log();
