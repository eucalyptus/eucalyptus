...
public class ChannelUtil {
  ...
  /** order really matters here. no touchy. **/
  ...
  public static ExecutorService getClientWorkerThreadPool( ) {
    ...
    try {
      if ( clientThreadPool == null ) {
        // TODO: decide on some parameters here.
        clientThreadPool = new OrderedMemoryAwareThreadPoolExecutor( CLIENT_POOL_MAX_THREADS, CLIENT_POOL_MAX_MEM_PER_CONN, CLIENT_POOL_TOTAL_MEM,
  ...
  public static ChannelFactory getClientChannelFactory( ) {
    ...
    try {
      if ( serverSocketFactory == null ) {
        serverSocketFactory = new NioClientSocketChannelFactory( sharedBossPool, clientThreadPool, Runtime.getRuntime( ).availableProcessors( ) * ( 2 + 1 ) );
      }
    }
    finally {
      canHas.unlock( );
  ...
  public static ChannelFactory getServerSocketChannelFactory( ) {
    ...
    try {
      if ( serverSocketFactory == null ) {
        serverSocketFactory = new NioServerSocketChannelFactory( sharedBossPool, serverWorkerThreadPool, Runtime.getRuntime( ).availableProcessors( )
                                                                                                         * ( 2 + 1 ) );
      }
    }
    finally {
   ...
   * 
   * If you need to broadcast a message to more than one Channel, you can add
   * the Channels associated with the recipients and call write(Object):
   * 
