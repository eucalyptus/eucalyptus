...
class UpdateCleanupUpdateMultiStepPromise extends MultiUpdateStepPromise {

  UpdateCleanupUpdateMultiStepPromise(
  ...
  protected UpdateStep getStep( final String stepId ) {
    return ((AWSCloudFormationStackResourceAction) resourceAction).getUpdateCleanupUpdateStep( stepId )
  }

  Promise<String> getUpdateCleanupUpdatePromise(String resourceId, String stackId, String accountId, String effectiveUserId, int resourceVersion ) {
    getPromise( "Resource ${resourceId} cleanup failed for stack ${stackId}" as String) { String stepId ->
      ...
      activities.performUpdateCleanupInnerStackUpdateStep(stepId, resourceId, stackId, accountId, effectiveUserId, resourceVersion - 1)
    }
  }
