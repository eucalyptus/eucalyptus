...
public class Clusters extends AbstractNamedRegistry<Cluster> {
  ...
  public void add( RegisterClusterType cluster ) {
  ...
  private ClusterInfo setupCluster( final RegisterClusterType c ) {
  ...
  private static void addClusterKeys( String name ) throws IOException, GeneralSecurityException {
    ...
    // if ( !serviceKs.containsEntry( ccAlias ) || !serviceKs.containsEntry(
    // ncAlias ) )
    // {
    ...
    // X509Certificate clusterX509 = keyTool.getCertificate( clusterKp,
    // EucalyptusProperties.getDName( "cc-" + name ) );
    // keyTool.writePem( SubDirectory.KEYS.toString() + File.separator +
    // "cluster-pk.pem", clusterKp.getPrivate() );
    // keyTool.writePem( SubDirectory.KEYS.toString() + File.separator +
    // "cluster-cert.pem", clusterX509 );
    // serviceKs.addKeyPair( ccAlias, clusterX509, clusterKp.getPrivate(),
    // ccAlias );
    //
    // KeyPair nodeKp = keyTool.getKeyPair();
    // X509Certificate nodeX509 = keyTool.getCertificate( nodeKp,
    // EucalyptusProperties.getDName( "nc-" + name ) );
    // keyTool.writePem( SubDirectory.KEYS.toString() + File.separator +
    // "node-pk.pem", nodeKp.getPrivate() );
    // keyTool.writePem( SubDirectory.KEYS.toString() + File.separator +
    // "node-cert.pem", nodeX509 );
