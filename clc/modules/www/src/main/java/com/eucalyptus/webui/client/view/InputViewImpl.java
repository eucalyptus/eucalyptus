...
public class InputViewImpl extends DialogBox implements InputView {
  ...
  private void displayFields( ArrayList<InputField> fields ) {
    ...
    for ( int i = 0; i < fields.size( ); i++ ) {
      ...
      grid.setHTML( i, 0, field.getTitle( ) );
      
      HasValueWidget widget = getHasValueWidget( field.getType( ) );
      inputs.add( widget );
      grid.setWidget( i, 1, widget.getWidget( ) );
      
      grid.setHTML( i, 2, "<div>&nbsp;</div>" );
    }
    
    this.contentPanel.setWidget( grid );
  }

  private HasValueWidget getHasValueWidget( ValueType type ) {
    switch ( type ) {
      case TEXT:
  ...
  private ArrayList<String> checkValues( ) {
    ...
    for ( int i = 0; i < inputs.size( ); i++ ) {
      ...
      if ( input != null ) {
        try {
          values.add( fields.get( i ).getChecker( ).check( input.getValue( ) ) );
