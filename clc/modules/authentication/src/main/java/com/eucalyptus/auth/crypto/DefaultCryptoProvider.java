...
public class DefaultCryptoProvider implements CryptoProvider, CertificateProvider, HmacProvider {
  ...
  public String generateQueryId( String userName ) {
    return this.getDigestBase64( userName, Digest.SHA224, false ).replaceAll( "\\p{Punct}", "" ).substring( 0, 20 );//NOTE: this MUST be 20-chars.
  }
  ...
  public String generateSecretKey( String userName ) {
    return this.getDigestBase64( userName, Digest.SHA384, true ).replaceAll( "\\p{Punct}", "" ).substring( 0, 40 );//NOTE: this MUST be 40-chars.
  }
