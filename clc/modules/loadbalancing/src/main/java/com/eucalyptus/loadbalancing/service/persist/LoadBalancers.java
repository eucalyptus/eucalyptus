...
public interface LoadBalancers {
  ...
  CompatFunction<LoadBalancerPolicyDescription, LoadBalancerPolicyDescriptionFullView>
      POLICY_DESCRIPTION_FULL_VIEW =
      policy -> ImmutableLoadBalancerPolicyDescriptionFullView.builder()
          ...
          .policyAttributeDescriptions(Stream.ofAll(policy.getPolicyAttributeDescriptions())
              .map(ImmutableLoadBalancerPolicyAttributeDescriptionView::copyOf))
          .build();

  CompatFunction<LoadBalancer, LoadBalancerView> CORE_VIEW = ImmutableLoadBalancerView::copyOf;

  CompatFunction<LoadBalancer, LoadBalancerZonesView> ZONES_VIEW =
      loadBalancer -> ImmutableLoadBalancerZonesView.builder()
          .loadBalancer(CORE_VIEW.apply(loadBalancer))
          .zones(Stream.ofAll(loadBalancer.getZones())
              .map(zone -> ImmutableLoadBalancerZoneFullView.builder()
                  .zone(ImmutableLoadBalancerZoneView.copyOf(zone))
                  .backendInstances(Stream.ofAll(zone.getBackendInstances())
                      .map(ImmutableLoadBalancerBackendInstanceView::copyOf))
                  .servoInstances(Stream.ofAll(zone.getServoInstances())
                      .map(ImmutableLoadBalancerServoInstanceView::copyOf))
                  .build()))
          .build();
  ...
  CompatFunction<LoadBalancer, LoadBalancerListenersView> LISTENERS_VIEW =
      loadBalancer -> ImmutableLoadBalancerListenersView.builder()
          .loadBalancer(CORE_VIEW.apply(loadBalancer))
          .listeners(Stream.ofAll(loadBalancer.getListeners())
              .map(listener -> ImmutableLoadBalancerListenerFullView.builder()
                  .listener(ImmutableLoadBalancerListenerView.copyOf(listener))
                  .policyDescriptions(Stream.ofAll(listener.getPolicyDescriptions())
                      .map(ImmutableLoadBalancerPolicyDescriptionView::copyOf))
                  .build()))
          .build();

  CompatFunction<LoadBalancer, LoadBalancerFullView> FULL_VIEW =
      loadBalancer -> ImmutableLoadBalancerFullView.builder()
          .loadBalancer(CORE_VIEW.apply(loadBalancer))
          .securityGroup(Option.of(loadBalancer.getSecurityGroup())
              .map(ImmutableLoadBalancerSecurityGroupView::copyOf)
              .getOrNull())
          .securityGroupRefs(Stream.ofAll(loadBalancer.getSecurityGroupRefs())
              .map(ImmutableLoadBalancerSecurityGroupRefView::copyOf))
          .autoScalingGroups(Stream.ofAll(loadBalancer.getAutoScaleGroups())
              .map(ImmutableLoadBalancerAutoScalingGroupView::copyOf))
          .backendInstances(Stream.ofAll(loadBalancer.getBackendInstances())
              .map(ImmutableLoadBalancerBackendInstanceView::copyOf))
          .backendServers(Stream.ofAll(loadBalancer.getBackendServers())
              .map(backendServer -> ImmutableLoadBalancerBackendServerDescriptionFullView.builder()
                  .backendServer(
                      ImmutableLoadBalancerBackendServerDescriptionView.copyOf(backendServer))
                  .policyDescriptions(Stream.ofAll(backendServer.getPolicyDescriptions())
                      .map(ImmutableLoadBalancerPolicyDescriptionView::copyOf))
                  .build()))
          .listeners(Stream.ofAll(loadBalancer.getListeners())
              .map(listener -> ImmutableLoadBalancerListenerFullView.builder()
                  ...
                  .build()))
          .policies(
              Stream.ofAll(loadBalancer.getPolicyDescriptions()).map(POLICY_DESCRIPTION_FULL_VIEW))
  ...
  @TypeMapper
  enum LoadBalancerToLoadBalancerAttributesTransform
      implements Function<LoadBalancerView, LoadBalancerAttributes> {
    INSTANCE;
