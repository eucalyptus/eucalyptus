...
public class ModifySnapshotAttributeType extends BlockSnapshotMessage {
  ...
  public List<CreateVolumePermissionItemType> asAddCreateVolumePermissionsItemTypes( ) {
    return !Strings.isNullOrEmpty( attribute ) ? "add".equals( operationType ) ? asCreateVolumePermissionItemTypes( ) : Lists.newArrayList( ) : createVolumePermission.getAdd( );
  }

  public List<CreateVolumePermissionItemType> asRemoveCreateVolumePermissionsItemTypes( ) {
    return !Strings.isNullOrEmpty( attribute ) ? "add".equals( operationType ) ? Lists.newArrayList( ) : asCreateVolumePermissionItemTypes( ) : createVolumePermission.getRemove( );
  }

  private List<CreateVolumePermissionItemType> asCreateVolumePermissionItemTypes( ) {
    return queryUserId.isEmpty( ) ? Stream.ofAll( queryUserGroup ).map( CreateVolumePermissionItemType.forGroup( ) ).toJavaList( ) : Stream.ofAll( queryUserId ).map( CreateVolumePermissionItemType.forUser( ) ).toJavaList( );
  }
