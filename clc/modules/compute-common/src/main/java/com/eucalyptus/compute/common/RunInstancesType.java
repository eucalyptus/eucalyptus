...
public class RunInstancesType extends VmControlMessage implements HasTags {
  ...
  public Set<String> securityGroupNames( ) {
    ...
    names.addAll( Stream.ofAll( groupIdSet ).filter( Strings.startsWith( "sg-" ).negate( ) ).toJavaList( ) ); // ID was historically the name
    names.addAll( Stream.ofAll( securityGroups ).map( GroupItemType.groupName( ) ).filter( CollectionUtils.isNotNull( ) ).toJavaList( ) );
    names.addAll( Stream.ofAll( securityGroups ).map( GroupItemType.groupId( ) ).filter( Strings.startsWith( "sg-" ).negate( ).and( CollectionUtils.isNotNull( ) ) ).toJavaList( ) );
    return names;
  }

  public Set<String> securityGroupsIds( ) {
    Set<String> names = Sets.newLinkedHashSet( );
    names.addAll( Stream.ofAll( groupIdSet ).filter( Strings.startsWith( "sg-" ) ).toJavaList( ) ); // ID was historically the name
    names.addAll( Stream.ofAll( securityGroups ).map( GroupItemType.groupId( ) ).filter( Strings.startsWith( "sg-" ) ).toJavaList( ) );
    return names;
  }
