...
public class ObjectReaperTask implements Runnable {
    ...
    public void reapPart(final PartEntity part) throws Exception {
        ...
        try {
            ...
            LOG.error("Unable to drop part: " + part.getBucketName() + " uploadId: " + part.getUploadId() + " partNumber: " + part.getPartNumber() + " uuid: " + part.getUuid());
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            List<PartEntity> partsToClean = ObjectManagers.getInstance().getDeletedParts();
            ...
            for(PartEntity part : partsToClean) {
                try {
                    ...
                    LOG.error("Error during part reaper cleanup for part: " +
                            part.getBucketName() + " uploadId: " + part.getUploadId() + " partNumber: " + part.getPartNumber() +
                            " uuid= " + part.getUuid(), f);
                }
            }

        } catch(final Throwable f) {
            LOG.error("Error during object/part reaper execution. Will retry later", f);
        } finally {
            try {
