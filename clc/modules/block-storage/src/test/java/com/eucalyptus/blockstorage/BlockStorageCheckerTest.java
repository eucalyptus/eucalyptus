...
public class BlockStorageCheckerTest {
  ...
  public void cleanFailedSnapshotsBasicTest() throws Exception {
    ...
    assertTrue("expected only one SnapshotInfo to still exist", remaining.size() == 1);
  }
  ...
  public void cleanFailedVolumeBasicTest() throws Exception {
    ...
    assertTrue("expected to have a result set querying the eucalyptus_storage persistence context", remaining != null);
    assertTrue("expected two VolumeInfos to still exist, but " + remaining.size() + " exist", remaining.size() == 2);
  }
  ...
  public void cleanFailedVolumesBasicTest() throws Exception {
    ...
    assertTrue("expected to have a result set querying the eucalyptus_storage persistence context", remaining != null);
    assertTrue("expected one VolumeInfo to still exist, but " + remaining.size() + " exist", remaining.size() == 1);
  }
