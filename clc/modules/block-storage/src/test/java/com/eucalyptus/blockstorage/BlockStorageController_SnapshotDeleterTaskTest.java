...
public class BlockStorageController_SnapshotDeleterTaskTest {
    ...
    public void run_BasicTest() throws Exception {
        ...
        BlockStorageController.SnapshotDeleterTask bscsdt =
                new BlockStorageController.SnapshotDeleterTask(
                        BlockStorageUnitTestSupport.createMockS3SnapshotTransfer()
                );
        bscsdt.run();
        ...
        assertTrue("expected to have a result set querying the eucalyptus_storage persistence context",
                remaining != null);
        assertTrue("expected all three SnapshotInfos to still exist",
                remaining.size() == 3);
        ...
        assertTrue("expected to have a result set querying the eucalyptus_storage persistence context for deleted snapshotinfos",
                remaining != null);
        assertTrue("expected two SnapshotInfos to exist",
                remaining.size() == 2);
