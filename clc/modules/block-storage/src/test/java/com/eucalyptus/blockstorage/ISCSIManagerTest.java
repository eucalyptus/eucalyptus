...
public class ISCSIManagerTest {
    ...
    public void allocateTarget_BasicTest() throws Exception {
        ...
        context.checking(new Expectations(){{
            oneOf(tgtService).targetExists(volId, 1, null, DirectStorageInfo.getStorageInfo().getTimeoutInMillis()); will( returnValue(Boolean.FALSE) );
        }});
        ...
        assertTrue("expected store number to be 2 but was " + retrieved.getStoreNumber(),
                retrieved.getStoreNumber().intValue() == 2);
        assertTrue("expected tid to be 2",
                retrieved.getTid().intValue() == 2);
        String storeName = "foo:" + StorageProperties.NAME + ":store2";
        assertTrue("expected iscsiVolumeInfo store name to be " + storeName,
                storeName.equals(iscsiVolumeInfo.getStoreName())) ;
        ...
        assertTrue("expected iscsiVolumeInfo tid to be 1 and lun to be 1",
                iscsiVolumeInfo.getTid().intValue() == 1 && iscsiVolumeInfo.getLun().intValue() == 1);
    }
    ...
    public void configure_BasicTest() throws Exception {
        ...
        final BlockStorageUtilSvc blockStorageUtilSvc = context.mock(BlockStorageUtilSvc.class);
        context.checking(new Expectations(){{
            oneOf(tgtService).userExists("eucalyptus", timeout); will(returnValue(Boolean.FALSE));
            oneOf(tgtService).addUser( with(equal("eucalyptus")), with(any(String.class)), with(equal(timeout)) );
            oneOf(blockStorageUtilSvc).encryptSCTargetPassword( with(any(String.class)) ); will(returnValue("foo"));
        }});
        ...
        assertTrue("expected store number to be 0 but it was " + retrieved.getStoreNumber().intValue(),
                retrieved.getStoreNumber().intValue() == 0);
        assertTrue("expected store user to be eucalyptus, but it was " + retrieved.getStoreUser(),
                "eucalyptus".equals(retrieved.getStoreUser()));
        assertTrue("expected tid to be 1, but it was " + retrieved.getTid().intValue(),
        ...
        assertTrue("expected eucalyptus chap user info to be created",
                "eucalyptus".equals(example.getUser()));
    }
    ...
    public void configure_NoDbUserButYesTGTUserTest() throws Exception {
        ...
        final BlockStorageUtilSvc blockStorageUtilSvc = context.mock(BlockStorageUtilSvc.class);
        context.checking(new Expectations(){{
            oneOf(tgtService).userExists( "eucalyptus", timeout ); will(returnValue(Boolean.TRUE));
    ...
    public void configure_YesDbUserButNoTGTUserTest() throws Exception {
        ...
        context.checking(new Expectations(){{
            oneOf(tgtService).userExists( "eucalyptus", timeout ); will(returnValue(Boolean.FALSE));
            oneOf(blockStorageUtilSvc).decryptSCTargetPassword("foo"); will(returnValue("foo"));
    ...
    public void cleanup_BasicTest() throws Exception {
        ...
        final Long timeout = DirectStorageInfo.getStorageInfo().getTimeoutInMillis();
        context.checking(new Expectations(){{
            oneOf(tgtService).targetExists(volId, 1, iscsiVolumeInfo.getAbsoluteLVPath(), timeout); will( returnValue(Boolean.TRUE) );
            oneOf(tgtService).targetExists(volId, 1, iscsiVolumeInfo.getAbsoluteLVPath(), timeout); will( returnValue(Boolean.TRUE) );
            ...
            oneOf(tgtService).targetExists(volId, 1, null, timeout); will( returnValue(Boolean.FALSE) );
            oneOf(tgtService).targetExists(volId, 1, iscsiVolumeInfo.getAbsoluteLVPath(), timeout); will( returnValue(Boolean.FALSE) );
        }});
        ...
        assertTrue("expected store name to be null but was " + retrieved.getStoreName(),
                retrieved.getStoreName() == null);
        assertTrue("expected tid to be -1",
                retrieved.getTid().intValue() == -1);
        assertTrue("expected iscsiVolumeInfo lun to be -1 but was " + retrieved.getLun().intValue(),
    ...
    public void getEncryptedPassword_BasicTest() throws Exception {
        ...
        context.checking(new Expectations(){{
            oneOf(blockStorageUtilSvc).decryptSCTargetPassword( "foo" ); will( returnValue("foo") );
            oneOf(blockStorageUtilSvc).getPartitionForLocalService(Storage.class); will( returnValue(null) );
            oneOf(blockStorageUtilSvc).encryptNodeTargetPassword( "foo", null ); will(returnValue("foo"));
        }});
        ...
        assertTrue("expected result to be 'foo' but was - " + foo,
                foo != null && "foo".equals(foo));
