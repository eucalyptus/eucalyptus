...
def filterComplex = { Field f -> filterFieldNames( f ) && !filterPrimitives( f ) && !filterCollections( f ) }
...
def renderSimpleField = { Field f ->
  """<xs:element name="${f.getName()}" type="xs:${xsTypeMap.get(f.getType().getCanonicalName())}" />"""
}

def renderComplexField = { Field f ->
  """<xs:element name="${f.getName()}" type="tns:${f.getType().getSimpleName()}Type" />"""
}

def renderCollectionField = { Field f ->
  """<xs:element name="${f.getName()}" type="tns:${transformToGenericType(f).getSimpleName()}SetType" minOccurs="0" />"""
}
