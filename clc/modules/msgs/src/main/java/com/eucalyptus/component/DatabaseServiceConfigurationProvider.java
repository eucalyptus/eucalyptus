...
public class DatabaseServiceConfigurationProvider<T extends ServiceConfiguration> implements ServiceConfigurationProvider<T> {
  ...
  public T store( T t ) throws ServiceRegistrationException {
    ...
    try {
      ...
      t = db.getUnique( t );
      db.commit( );
      EventRecord.here( ServiceConfigurationProvider.class, EventClass.COMPONENT, EventType.COMPONENT_REGISTERED ).withDetails( t.getComponent( ).name( ), t.getName( ), "host", t.getHostName( ) );
    } catch ( Exception e ) {
      db.rollback( );
      LOG.error( e, e );
  ...
  public T remove( T t ) throws ServiceRegistrationException {
    ...
    try {
      ...
      db.delete( exists );
      db.commit( );
      EventRecord.here( ServiceConfigurationProvider.class, EventClass.COMPONENT, EventType.COMPONENT_DEREGISTERED ).withDetails( t.getComponent( ).name( ), t.getName( ), "host", t.getHostName( )  );
    } catch ( Exception e ) {
      db.rollback( );
      LOG.error( e, e );
      throw new ServiceRegistrationException( e );
