...
public class DatabaseUtil implements EventListener {
  ...
  public enum PoolConfig {
    acquire_increment( 5 ), idle_test_period( 0 ), timeout( 30000 ), max_size( 32 ), max_statements( 0 ), min_size( 2 );
    int value;
  ...
  public static void printConnectionPoolStatus( ) {
    for ( PooledDataSource ds : ( Set<PooledDataSource> ) C3P0Registry.getPooledDataSources( ) ) {
      try {
        ...
        LOG.debug( String.format( "-> Threads: size=%d active=%d idle=%d pending=%d helpers=%d waiting=%d", ds.getThreadPoolSize( ), ds.getThreadPoolNumActiveThreads( ),
                                  ds.getThreadPoolNumIdleThreads( ), ds.getThreadPoolNumTasksPending( ), ds.getNumHelperThreads( ), ds.getNumThreadsAwaitingCheckoutDefaultUser( ) ) );
        LOG.debug( String.format( "-> Statement Cache: size=%d conn-w/-stmts=%d checked-out=%d", ds.getStatementCacheNumStatementsAllUsers( ),
                                  ds.getStatementCacheNumConnectionsWithCachedStatementsAllUsers( ), ds.getStatementCacheNumCheckedOutStatementsAllUsers( ) ) );
        LOG.debug( String.format( "-> Connections: size=%d busy=%d idle=%d orphan=%d failed-checkin=%d failed-checkout=%d failed-idle=%d", ds.getNumConnectionsAllUsers( ),
                                  ds.getNumBusyConnectionsAllUsers( ), ds.getNumIdleConnectionsAllUsers( ), ds.getNumUnclosedOrphanedConnectionsAllUsers( ),
