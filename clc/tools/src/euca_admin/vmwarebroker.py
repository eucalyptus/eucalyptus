...
class VMwareBroker():
  ...
  def get_register_parser(self):
    ...
    parser.add_option("-H","--host",dest="host",
                      help="Hostname of the vmwarebroker.")
    parser.add_option("-p","--port",dest="port",type="int",
                      default=8773,help="Port for the vmwarebroker.")
    parser.add_option("-P","--partition",dest="partition",
                      help="Partition for the vmwarebroker.")
    (options,args) = parser.parse_args()    
  ...
  def register(self, host, name='vmwarebroker', port=8773, partition=None):
    if host == None:
      self.euca.handle_error("Missing hostname")
    params = {'Name':'vmwarebroker',
              'Host':host,
              'Port':port}
    if partition:
      params['Partition'] = partition
    try:
      reply = self.euca.connection.get_object('RegisterVMwareBroker',
                                              params,
                                              BooleanResponse)
      print reply
    except EC2ResponseError, ex:
  ...
  def deregister(self, name, partition='vmwarebroker'):
    params = {'Name':name}
    if partition:
      params['Partition'] = partition
    try:
      reply = self.euca.connection.get_object('DeregisterVMwareBroker',
  ...
  def get_modify_parser(self):
    ...
    (options,args) = parser.parse_args()    
    if len(args) != 1:
      print "ERROR  Required argument VMWARENAME is missing or malformed."
      parser.print_help()
      sys.exit(1)
  ...
  def modify(self,partition,name,modify_list):
    for entry in modify_list:
      ...
      try:
        reply = self.euca.connection.get_object('ModifyVMwareBrokerAttribute',
