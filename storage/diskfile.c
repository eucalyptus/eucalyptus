...
diskfile * df_create (const char * path, const long long limit_bytes, boolean zero_fill)
{
	if (!help_initialized) {
		if (help_init()) {
			logprintfl (EUCAERROR, "error: failed to initialize helpers\n");
			return NULL;
		}
	...
	if (help_ddzero (path, df->size_sectors, zero_fill)!=OK) {
		free (df);
		df = NULL;
	}
...
diskfile * df_open (const char * path)
{
	...
	for (int p = 0; p < MAX_PARTS; p++) {
		...
		if (help_sectors (df->path, p, &first, &last)==OK) {
...
int df_partition (diskfile * df, enum mbr_t type, diskpart parts[])
{
	...
	if (help_mbr (df->path, "msdos")!=OK) goto error;
	for (int i=0; i<df->nparts; i++) {
		...
		if (help_part (df->path, "primary", enum_diskpart_t_string (q->type), q->first_sector, q->last_sector)!=OK) goto error;
	}
...
int df_dd (diskfile * df, const int part, const char * path) 
{
	...
	if (help_dd (path, p->lodev, SECTOR_SIZE, (p->last_sector - p->first_sector + 1))!=OK) { // TODO: experiment with block sizes?
...
long long get_min_size (enum diskpart_t t)
{
	static long long _diskparts [sizeof (enum diskpart_t)] = {
		0,  // unknown
		80, // linux-swap: based on weak empirical evidence
		40, // ext2: based on weak empirical evidence
		0
	};
