...
int do_convert(char *infile, char *outfile, char *kernel, char *ramdisk, char *modules, int swap, int ephemeral, int bits, int dopause, int force) {
  ...
  char dtemplate[32] = "/tmp/ami2vmx.XXXXXX", file[1024], cmd[1024];
  ...
  // now we have a disk image
  {
    ...
    snprintf (file, 1024, "%s/boot/grub/menu.lst", tmpdir);
    FH = fopen(file, "w");
    if (ramdisk) {
  ...
  /*
  // all of the helpers currently defined
  for (i=0; i<LASTHELPER; i++) {
    logprintfl (EUCAINFO, "Helper: %s\n", helpers_path[i]);
  }
  
  // all verified input params
  logprintfl (EUCAINFO, "Input image: %s\n", infile);
  logprintfl (EUCAINFO, "Output image: %s\n", outfile);
  logprintfl (EUCAINFO, "Kernel: %s\n", kernel);
  if (ramdisk) logprintfl (EUCAINFO, "Ramdisk: %s\n", ramdisk);
  logprintfl (EUCAINFO, "Modules: %s\n", modules);

  // Example of running one of the helpers
  {
    char *output;
    output = pruntf("%s -h", helpers_path[MOUNT]);
    if (output) {
      logprintfl (EUCAINFO, "\nOUTPUT OF CMD\n---------------\n");
      logprintfl (EUCAINFO, "%s", output);
      free(output);
    }
  }
  */
