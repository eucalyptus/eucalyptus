...
int do_metaproxy_maintain(mido_config *mido, int mode) {
  ...
  if (dorun) {
    if (mode == 0) {
      ...
      snprintf(cmd, EUCA_MAX_PATH, "nginx -p . -c %s/usr/share/eucalyptus/nginx_proxy.conf -g 'pid %s/var/run/eucalyptus/nginx_localproxy.pid; env NEXTHOP=127.0.0.1; env NEXTHOPPORT=8773; env EUCAHOME=%s;'", mido->eucahome, mido->eucahome, mido->eucahome);
    } else if (mode == 1) {
  ...
  for (i=0; i<mido->max_vpcs; i++) {
	...
	snprintf(cmd, EUCA_MAX_PATH, "ip netns exec %s nginx -p . -c %s/usr/share/eucalyptus/nginx_proxy.conf -g 'pid %s/var/run/eucalyptus/nginx_vpcproxy_%s.pid; env VPCID=%s; env NEXTHOP=169.254.0.1; env NEXTHOPPORT=31338; env EUCAHOME=%s;'", mido->vpcs[i].name, mido->eucahome, mido->eucahome, mido->vpcs[i].name, mido->vpcs[i].name, mido->eucahome);
      } else if (mode == 1) {
  ...
  for (i=0; i<gni->max_instances; i++) {
	...
	if (vpcsubnet) {
	  ...
	  // do sec. group rule application for instance
	  {
	    ...
	    rc = mido_create_rule(&(vpcinstance->midos[INST_PRECHAIN]), NULL, "position", tmp_name3, "type", "dnat", "flowAction", "continue", "ipAddrGroupDst", mido->midocore->midos[METADATA_IPADDRGROUP].uuid, "nwProto", "6", "tpDst", "jsonjson", "tpDst:start", "80", "tpDst:end", "80", "tpDst:END", "END", "natTargets", "jsonlist", "natTargets:addressTo", pt_buf, "natTargets:addressFrom", pt_buf, "natTargets:portFrom", "31337", "natTargets:portTo", "31337", "natTargets:END", "END", NULL);
	    if (rc) {
	    } else {
	      rulepos++;
	    }
	    ...
	    //#if 0
	    snprintf(tmp_name3, 32, "%d", rulepos);
	    rc = mido_create_rule(&(vpcinstance->midos[INST_POSTCHAIN]), NULL, "position", tmp_name3, "type", "snat", "flowAction", "continue", "nwSrcAddress", pt_buf, "nwSrcLength", "32", "nwProto", "6", "tpSrc", "jsonjson", "tpSrc:start", "31337", "tpSrc:end", "31337", "tpSrc:END", "END", "natTargets", "jsonlist", "natTargets:addressTo", "169.254.169.254", "natTargets:addressFrom", "169.254.169.254", "natTargets:portFrom", "80", "natTargets:portTo", "80", "natTargets:END", "END", NULL);
	    if (rc) {
	    } else {
	      rulepos++;
	    }
		...
		EUCA_FREE(rules);
	      ...
	      snprintf(tmp_name3, 32, "%d", rulepos);
	      rc = mido_create_rule(&(vpcinstance->midos[INST_POSTCHAIN]), NULL, "position", tmp_name3, "type", "jump", "jumpChainId", vpcsecgroup->midos[VPCSG_INGRESS].uuid, NULL);
	      if (rc) {
	      } else {
		rulepos++;
	      }
		...
		if (strlen(gnisecgroup->ingress_rules[k].groupId)) {
		  ...
		  snprintf(tmp_name3, 32, "%d", rulepos);
		  rc = mido_create_rule(&(vpcsecgroup->midos[VPCSG_INGRESS]), NULL, "position", tmp_name3, "type", "accept", "tpDst", "jsonjson", "tpDst:start", tmp_name1, "tpDst:end", tmp_name2, "tpDst:END", "END", "nwProto", tmp_name4, NULL);
		  if (rc) {
		  } else {
		    rulepos++;
		  }
		  ...
		  if (gnisecgroup->ingress_rules[k].icmpCode >= 0) {
		    ...
		    rc = mido_create_rule(&(vpcsecgroup->midos[VPCSG_INGRESS]), NULL, "position", tmp_name3, "type", "accept", "tpDst", "jsonjson", "tpDst:start", tmp_name1, "tpDst:end", tmp_name1, "tpDst:END", "END", "nwProto", tmp_name4, NULL);
		    if (rc) {
		    } else {
		      rulepos++;
		    }
...
void print_mido_vpc_subnet(mido_vpc_subnet *vpcsubnet) {
  LOGDEBUG("PRINT VPCSUBNET: name=%s vpcname=%s max_brports=%d max_dhcphosts=%d max_instances=%d gnipresent=%d\n", vpcsubnet->name, vpcsubnet->vpcname, vpcsubnet->max_brports, vpcsubnet->max_dhcphosts, vpcsubnet->max_instances, vpcsubnet->gnipresent);
...
int populate_mido_vpc(mido_config *mido, mido_core *midocore, mido_vpc *vpc) {
  ...
  {
    // temporary
    LOGDEBUG("AFTER POPULATE: VPCRT: %d VPCRT_PRECHAIN: %d VPCRT_POSTCHAIN: %d EUCABR_DOWNLINK: %d VPCRT_UPLINK: %d\n", vpc->midos[VPCRT].init, vpc->midos[VPCRT_PRECHAIN].init, vpc->midos[VPCRT_POSTCHAIN].init, vpc->midos[EUCABR_DOWNLINK].init, vpc->midos[VPCRT_UPLINK].init);
  }
...
int populate_mido_core(mido_config *mido, mido_core *midocore) {
  ...
  {
    // temporary
    LOGDEBUG("AFTER POPULATE: EUCART: %d EUCABR: %d EUCART_BRPORT: %d EUCABR_RTPORT: %d EUCART_GWPORT: %d GWHOST: %d\n", midocore->midos[EUCART].init, midocore->midos[EUCABR].init, midocore->midos[EUCART_BRPORT].init, midocore->midos[EUCABR_RTPORT].init, midocore->midos[EUCART_GWPORT].init, midocore->midos[GWHOST].init);
  }
...
int connect_mido_vpc_instance_elip(mido_config *mido, mido_core *midocore, mido_vpc *vpc, mido_vpc_subnet *vpcsubnet, mido_vpc_instance *vpcinstance) {
  ...
  */
  ...
  rc = mido_create_rule(&(vpc->midos[VPCRT_PREELIPCHAIN]), &(vpcinstance->midos[ELIP_PRE]), "type", "dnat", "flowAction", "continue", "ipAddrGroupDst", vpcinstance->midos[ELIP_PRE_IPADDRGROUP].uuid, "natTargets", "jsonlist", "natTargets:addressTo", ipAddr_priv, "natTargets:addressFrom", ipAddr_priv, "natTargets:portFrom", "0", "natTargets:portTo", "0", "natTargets:END", "END", NULL);
  if (rc) {
