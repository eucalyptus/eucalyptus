...
int cmp_edge_config(edge_config *a, edge_config *b, int *my_instances_diff,
    ...
    // Compare referenced security groups
    //  - We assume the ref_sgs are sorted identically
    //  - eucanetd only supports one level of indirection, more would (maybe?) require loop detection 
    //      - would need to be implemented in gni_get_referenced_secgroups
    if (abmatch) { 
        if(a->ref_sgs && b->ref_sgs) {
        ...
        } else if ( (a->ref_sgs && !(b->ref_sgs)) || (!(a->ref_sgs) && b->ref_sgs) ) {
            abmatch = 0;
        }
    }
    if (!abmatch && sgs_diff) {
