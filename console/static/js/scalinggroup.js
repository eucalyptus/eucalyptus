...
(function($, eucalyptus) {
  $.widget('eucalyptus.scalinggroup', $.eucalyptus.eucawidget, {
    options : { },
    baseTable : null,
    tableWrapper : null,
    delDialog : null,
    ...
    attachDialog : null,
    tagDialog : null,
    attachButtonId : 'scalinggrp-attach-btn',
    createButtonId : 'scalinggrps-add-btn',
    _init : function() {
      var thisObj = this;
      var $tmpl = $('html body').find('.templates #scalingGrpTblTmpl').clone();
      ...
      this.tableWrapper = $volTable.eucatable({
        id : 'scalinggrps', // user of this widget should customize these options,
    ...
    _create : function() { 
      ...
      var option = {
         ...
         on_open: {spin: true, callback: [ function(args) {
           ...
           $add_dialog.eucadialog('buttonOnChange', $az_selector, thisObj.createButtonId, function(){
             return $az_selector.val() !== '' &&  $vol_size_edit.val() == parseInt(asText($vol_size_edit.val()));
           }); 
           $add_dialog.eucadialog('buttonOnKeyup', $vol_size_edit, thisObj.createButtonId, function(){
             return $az_selector.val() !== '' &&  $vol_size_edit.val() == parseInt(asText($vol_size_edit.val()));
           });
      ...
      $snapSelector.change( function(){
        ...
        if (snapshotId) {
          ...
          if (thisObj.addDialog.find('#volume-add-az-selector').val() != '')
            thisObj.addDialog.eucadialog('enableButton', thisObj.createButtonId);
        }
      });
