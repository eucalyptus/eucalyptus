    ...
    return EucaDialogView.extend({
        initialize : function(args) {
            ...
            this.scope = {
                ...
                deleteButton: {
                  click: function() {
	            // FOR EACH SNAPSHOT IN THE LIST 'items'
	            _.each(self.scope.items, function(item) {
		      console.log("Snapshot Item: " + item);
		      // FIND THE MODEL THAT MATCHES THE SNAPSHOT ID WITH 'item'
		      var match = App.data.snapshot.find(function(model){ return model.get('id') == item; });
		        ...
		        success: function(data, response, jqXHR){   // AJAX CALL SUCCESS OPTION
		          ...
		          if(data.results){
		            console.log("sync: Snapshot " +item+ " Deleted = " + data.results );
		      ...
		      console.log("Snapshot After Delete: " + item.get('id'));
	            });
