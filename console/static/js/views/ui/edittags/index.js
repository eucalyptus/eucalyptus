        ...
        model.on('confirm', function(defer) {
          ...
          _.chain(tags.models).clone().each(function(t) {
            ...
            var d_options = {
              success: function(model, response, options){
                if(model != null){
                  notifySuccess(null, $.i18n.prop('tag_delete_success', name, res_name));
                }else{
                  notifyError($.i18n.prop('tag_delete_error', name, res_name), undefined_error);
                }
              },
              error: function(model, jqXHR, options){
                notifyError($.i18n.prop('tag_delete_error', name, res_name), getErrorMessage(jqXHR));
              }
            };
            if (t.get('_deleted')) {
